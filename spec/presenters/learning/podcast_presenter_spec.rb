...
RSpec.describe Learning::PodcastPresenter do
  subject(:presenter) { described_class.new(learning_podcast) }

  let(:url) { FFaker::Internet.http_url }
  let(:title) { FFaker::Lorem.sentence }
  let(:learning_podcast) { create(:learning_podcast, url: url, title: "<strong>a</strong>#{title}") }
